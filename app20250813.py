...
st.set_page_config(page_title="TSMC 一週新聞情緒監測", page_icon="📈", layout="wide")
...
def to_dt(ts):
    try:
        return dt.datetime.utcfromtimestamp(int(ts)).replace(tzinfo=dt.timezone.utc).astimezone()
    except Exception:
        return pd.NaT
...
if run:
    ...
    col1.metric("近一週樣本數", len(dfN))
    col2.metric("加權情緒分數", f"{overall_score:.3f}")
    col3.metric("一週傾向", f"{badge_color} {outlook}")
